...
export const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant = "default", size = "md", loading, children, disabled, ...props }, ref) => {
    ...
    const variants = {
      default: "bg-red-500 text-white hover:bg-red-600 active:bg-red-700 shadow-lg shadow-red-500/20",
      ghost: "text-zinc-400 hover:text-white hover:bg-white/5",
      outline: "border border-zinc-700 text-zinc-300 hover:border-zinc-500 hover:text-white hover:bg-white/5",
      destructive: "bg-red-900/40 text-red-400 border border-red-900 hover:bg-red-900/60 hover:text-red-300",
...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    >
      {children}
      <DialogPrimitive.Close className="absolute right-4 top-4 rounded-lg p-1 text-zinc-500 hover:text-white hover:bg-zinc-800 transition-colors">
        <X size={16} />
...
export function StatCard({ title, value, sub, icon, trend, color = "red" }: StatCardProps) {
  const colors = {
  ...
  return (
    <div className={cn(
      "relative rounded-2xl border bg-gradient-to-br p-5 transition-all duration-200 stat-card-glow",
      colors[color]
    )}>
      <div className="flex items-start justify-between">
        <div>
          <p className="text-xs font-mono font-medium text-zinc-500 uppercase tracking-widest">{title}</p>
      ...
      {trend && (
        <div className="mt-3 flex items-center gap-1.5">
          <span className={cn("text-xs font-mono", trend.value >= 0 ? "text-emerald-400" : "text-red-400")}>
...
export function TableHead({ className, ...props }: React.ThHTMLAttributes<HTMLTableCellElement>) {
  return (
    <th
      className={cn(
        "h-11 px-4 text-left align-middle text-xs font-mono font-medium text-zinc-600 uppercase tracking-widest",
        className
      )}
      {...props}
    />
...
export function TableCell({ className, ...props }: React.TdHTMLAttributes<HTMLTableCellElement>) {
  return (
    <td className={cn("px-4 py-3 align-middle text-sm text-zinc-300", className)} {...props} />
  );
}
...
export function Avatar({ name = "?", size = "md", color }: AvatarProps) {
  ...
  const colors = ["bg-red-900/60", "bg-blue-900/60", "bg-purple-900/60", "bg-emerald-900/60", "bg-amber-900/60"];
  const bg = color || colors[name.charCodeAt(0) % colors.length];
  return (
    <div className={cn("rounded-xl flex items-center justify-center font-semibold text-white", sizes[size], bg)}>
      {initials}
    </div>
  );
}
